	...
	void AidMan::run() {
		...
		do
		{
			...
			case 6:
			{
				...
				out << "Shipping Order, Date: " << Date() << endl;
				out << " ROW |  SKU  | Description                         | Have | Need |  Price  | Expiry" << endl;
				out << "-----+-------+-------------------------------------+------+------+---------+-----------" << endl;
				for (int i = 0; i < counter; i++)
				{
					if (arr[i]->qty() == arr[i]->qtyNeeded())
					{
						out << "   " << fixed << right << c + 1 << " | ";
						arr[i]->linear(true);
						arr[i]->display(out);
				...
				out << "-----+-------+-------------------------------------+------+------+---------+-----------" << endl;
				cout << "Shipping Order for "<< c <<" times saved!" << endl<< endl;
				break;
			}
			case 7:
